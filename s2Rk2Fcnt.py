...
def zerothVector(firstVec, secondVec, g):
    ...
    M0.append(
        -1j * g * np.abs(secondVec[N - 1]) ** 2 * firstVec[N - 1]
    )
...
def firstVector(firstVec, secondVec, g, K0, M0):
    ...
    M1 = []
    K1.append(
        -1j * g * np.abs(firstVec[0] + 1 / 2 * dt * K0[0]) ** 2 * (secondVec[0] + 1 / 2 * dt * M0[0])
    )
    for n in range(1, N):
        K1.append(
            -1j * g * np.abs(firstVec[n] + 1 / 2 * dt * K0[n]) ** 2 * (
                        secondVec[n - 1] + 1 / 2 * dt * M0[n - 1] + secondVec[n] + 1 / 2 * dt * M0[n])
        )
    for n in range(0, N - 1):
        M1.append(
            -1j * g * np.abs(secondVec[n] + 1 / 2 * dt * M0[n]) ** 2 * (
                        firstVec[n] + 1 / 2 * dt * K0[n] + firstVec[n + 1] + 1 / 2 * dt * K0[n + 1])
        )
    M1.append(
        -1j * g * np.abs(secondVec[N - 1] + 1 / 2 * dt * M0[N - 1]) ** 2 * (firstVec[N - 1] + 1 / 2 * dt * K0[N - 1])
    )
    return K1, M1
